...
from hem_core.input_output.enums import ZoneTemperatureControlBasis
...
class Zone:
    ...
    def __init__(
        ...
        self.__area_el_total = fsum([eli.area for eli in self.__building_elements])
    ...
    @cython.cfunc
    def __init_node_temps(
        self, temp_ext_air_init: cython.double, temp_setpnt_init: cython.double
    ) -> cython.void:
        ...
        while True:
            ...
            gains_heat_cool: cython.double = (
                (space_heat_demand + space_cool_demand) * units.W_per_kW / delta_t_h
            )
    ...
    def __calc_temperatures(
        ...
        for eli in self.__building_elements:
            ...
            vector_b[idx] = (
                (eli.k_pli[i] / delta_t) * temp_prev[idx]
            ...
            # Coeff for temperature of this node
            matrix_a[idx][idx] = (
                (eli.k_pli[i] / delta_t) + h_ci + eli.h_ri() * sum_area_frac + eli.h_pli(i - 1)
            )
            ...
            for elk in self.__building_elements:
                ...
                matrix_a[idx][col] = (
                    matrix_a[idx][col] - (elk.area / self.__area_el_total) * eli.h_ri()
                )
        ...
        matrix_a[self.__zone_idx][self.__zone_idx] = (
            ...
            + fsum(
                [
                    eli.area
                    * eli.h_ci(
                        temp_prev[self.__zone_idx], temp_prev[self.__element_positions[eli][1]]
                    )
                    for eli in self.__building_elements
                ]
            )
        ...
        if print_heat_balance:
            ...
            hb_energy_to_change_temp = -(self.__c_int / delta_t) * (
                temp_internal - temp_prev[self.__zone_idx]
            )
            ...
            for eli in self.__building_elements:
                ...
                hb_fabric_ext_air_radiative += (
                    eli.area * (eli.h_re()) * (eli.temp_ext() - temp_ext_surface)
                )
                hb_fabric_ext_sol += (
                    eli.area
                ...
                hb_fabric_ext = (
                    eli.area * ((eli.h_ce()) * (eli.temp_ext() - temp_ext_surface))
                    + eli.area * (eli.h_re()) * (eli.temp_ext() - temp_ext_surface)
                    + eli.area
                    * eli.solar_absorption_coeff
                    * (i_sol_dif * f_sh_dif + i_sol_dir * f_sh_dir)
                ...
                elif type(eli) in (BuildingElementPartyWall,):
                    hb_fabric_ext_ZTU += hb_fabric_ext
    ...
    def __temp_operative(self, temp_vector: cython.double[:]) -> cython.double:
        ...
        temp_mean_radiant: cython.double = (
            fsum(
                [
                    eli.area * temp_vector[self.__element_positions[eli][1]]
                    for eli in self.__building_elements
                ]
            )
            / self.__area_el_total
    ...
    def ___ach_req_to_reach_temperature(
        ...
        if (
            temp_ach_max >= temp_ach_min
            or temp_int_air_max >= temp_int_air_min
    ...
    def __calc_cooling_potential_from_ventilation(
        ...
        if ach_cooling is None and ach_windows_open != ach_target:
            ...
            temp_vent_max: cython.double
            if self.__temp_setpnt_basis == ZoneTemperatureControlBasis.OPERATIVE:
                temp_vent_max = self.__temp_operative(temp_vector_vent_max)
            elif self.__temp_setpnt_basis == ZoneTemperatureControlBasis.AIR:
                temp_vent_max = temp_int_air_vent_max
            ...
            if (
                temp_vent_max < temp_free
                and temp_free > temp_setpnt_cool_vent
                ...
                temp_free_vent_extra: cython.double
                if self.__temp_setpnt_basis == ZoneTemperatureControlBasis.OPERATIVE:
                    temp_free_vent_extra = self.__temp_operative(
                        temp_vector_no_heat_cool_vent_extra
                    )
                elif self.__temp_setpnt_basis == ZoneTemperatureControlBasis.AIR:
                    temp_free_vent_extra = temp_vector_no_heat_cool_vent_extra[self.__zone_idx]
        ...
        return temp_free, ach_cooling, ach_to_trigger_heating
    ...
    def __interp_heat_cool_demand(
        ...
        if math.isclose(temp_upper - temp_free, 0.0, abs_tol=1e-10):
            raise ValueError(zero_division_error_msg)

        heat_cool_load_unrestricted: cython.double = (
            heat_cool_load_upper * (temp_setpnt - temp_free) / (temp_upper - temp_free)
        )

        # Convert from W to kWh
        heat_cool_demand: cython.double = heat_cool_load_unrestricted / units.W_per_kW * delta_t_h
    ...
    def space_heat_cool_demand(
        ...
        ach_cooling: "cython.double | None" = None,
    ) -> tuple:
        ...
        if temp_setpnt_cool < temp_setpnt_heat:
            raise ValueError("Cooling setpoint is below heating setpoint.")
        ...
        if self.__control_obj is not None:
            ...
            if temp_setpnt_cool_vent_response < temp_setpnt_heat:
                raise ValueError("Setpoint for additional ventilation is below heating setpoint.")
        ...
        if temp_setpnt_cool_vent < temp_setpnt_heat:
            raise ValueError("Cooling ventilation setpoint is below heating setpoint.")
        ...
        if math.isclose(gains_heat_cool, 0.0, abs_tol=1e-10):
            frac_conv_gains_heat_cool: cython.double = 0.0
        else:
            frac_conv_gains_heat_cool: cython.double = gains_heat_cool_convective / gains_heat_cool
        ...
        temp_free: cython.double
        if self.__temp_setpnt_basis == ZoneTemperatureControlBasis.OPERATIVE:
            temp_free = self.__temp_operative(temp_vector_no_heat_cool)
        elif self.__temp_setpnt_basis == ZoneTemperatureControlBasis.AIR:
            temp_free = temp_int_air_free
        ...
        if temp_free > temp_setpnt_cool and not math.isclose(
            temp_free, temp_setpnt_cool, abs_tol=1e-10
        ):
            # Cooling
        ...
        elif temp_free < temp_setpnt_heat and not math.isclose(
            temp_free, temp_setpnt_heat, abs_tol=1e-10
        ):
            # Heating
            # TODO Implement eqn 26 "if max power available" case rather than just "otherwise" case?
            #      Could max. power be available at this point for all heating/cooling systems?
        ...
        frac_convective_heat_cool_upper: cython.double = (
            gains_heat_cool * frac_conv_gains_heat_cool + heat_cool_load_upper * frac_convective
        ...
        temp_upper: cython.double
        if self.__temp_setpnt_basis == ZoneTemperatureControlBasis.OPERATIVE:
            # Calculate internal operative temperature with maximum heating/cooling
            temp_upper = self.__temp_operative(temp_vector_upper_heat_cool)
        elif self.__temp_setpnt_basis == ZoneTemperatureControlBasis.AIR:
            temp_upper = temp_vector_upper_heat_cool[self.__zone_idx]
