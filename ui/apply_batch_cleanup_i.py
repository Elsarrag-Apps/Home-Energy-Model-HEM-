...
def extract_space_heating_systems_from_hem(hem_json_path: Path) -> dict:
    """Extract the SpaceHeatSystem dictionary from uploaded HEM JSON."""
    hem_input = load_json_file(hem_json_path)
    return hem_input.get("SpaceHeatSystem", {})


def summarise_space_heating_systems(space_heat_systems: dict) -> list[dict]:
    """Create a summary table for HEM SpaceHeatSystem entries."""
    rows = []
            ...
            rows.append(
                {
                    ...
                    "energy_supply": system.get(
                        "EnergySupply",
                        system.get("energy_supply", ""),
                    ),
                    "priority": system.get("priority", ""),
...
def apply_space_heating_to_case(
    hem_input: dict,
    space_heat_systems: dict,
) -> dict:
    """Apply saved SpaceHeatSystem dictionary to a HEM input."""
    if not space_heat_systems:
        return hem_input

    if not isinstance(space_heat_systems, dict):
        return hem_input
...
print("Batch cleanup I complete.")